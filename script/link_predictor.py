...
class LinkPredictor:
    def __init__(self, model, owl_file):
        """
    ...
    def get_book_nodes(self):
        """Extract all books (instances of Book) from the OWL file using rdflib"""
        g = Graph()
    ...
    def hit_at_k(ground_truth, predictions, k):
        ...
        for node, true_items in ground_truth.items():
            ...
            predicted_items = {p[0] for p in predictions.get(node, [])[:k]}
    ...
    def mean_reciprocal_rank(ground_truth, predictions):
        mrr = 0
        for node, true_items in ground_truth.items():
            predicted_items = [p[0] for p in predictions.get(node, [])]
