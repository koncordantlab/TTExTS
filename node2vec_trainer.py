...
class Node2Vec:
    def __init__(
        ...
        self.seed = seed
        # Local RNG instances. `random.Random(None)` and `np.random.default_rng(None)`
        # fall back to system entropy, matching the old None-seed behavior.
    ...
    def _alias_draw(self, J: np.ndarray, q: np.ndarray) -> int:
        """
        Seeded version of alias_draw that uses this instance's NumPy RNG.
        Identical logic to the module-level alias_draw function, but
        sources randomness from self.np_rng so walks are reproducible.
        """
        K = len(J)
        kk = int(np.floor(self.np_rng.random() * K))
        if self.np_rng.random() < q[kk]:
            return kk
        else:
            return int(J[kk])
    ...
    def _edge_sample_next(self, prev: Any, curr: Any) -> Optional[Any]:
        ...
        if key not in self.alias_edges:
            # fallback to uniform neighbor sampling (seeded)
            return self.py_rng.choice(neighbors)
